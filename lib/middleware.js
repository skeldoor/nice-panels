...
// Map old tier names to their current equivalents
...
function attachUser(req, res, next) {
    ...
    if (token) {
        ...
        if (decoded) {
            // Migrate old tier names from stale JWTs
...
function requireAuth(req, res, next) {
    ...
    // Bypass auth for requests with a valid API key (e.g. AI assistants)
    const apiKey = req.headers['x-api-key'];
    if (apiKey && apiKey === process.env.API_BYPASS_KEY) {
        req.user = { patreonId: 'api-client', name: 'API Client', tier: 'full' };
        return next();
    }
