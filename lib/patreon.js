...
async function getPatronInfo(accessToken) {
    ...
    // Admin override — creators/admins get the highest tier automatically (hardcoded)
    ...
    // Manual overrides from Redis (set via admin dashboard)
    const { getOverride } = require('./redis');
    try {
        const overrideTier = await getOverride(user.id);
        if (overrideTier && tierConfig.tierOrder.includes(overrideTier)) {
            return { user, tier: overrideTier };
        }
    } catch (err) {
        console.error('Failed to check Redis override for user', user.id, err);
        // Fall through to Patreon check if Redis fails
    }
