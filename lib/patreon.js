...
async function getPatronInfo(accessToken) {
    ...
    // Manual overrides — hardcoded access grants (highest checked first)
    const manualFull = tierConfig.manualFull || [];
    if (manualFull.includes(user.id)) {
        return { user, tier: 'full' };
    }
    const manualEnhanced = tierConfig.manualEnhanced || [];
    if (manualEnhanced.includes(user.id)) {
        return { user, tier: 'enhanced' };
    }
    const manualBasic = tierConfig.manualBasic || [];
    if (manualBasic.includes(user.id)) {
