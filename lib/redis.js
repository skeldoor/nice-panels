...
const TIER_ALLOWANCES = {
    basic: 1,
    enhanced: 3,
    full: 7, // all panels — keep in sync with config/tiers.json tools count
};
 ...
 * For 'full' tier, returns the total number of tools so it always matches.
 */
function getTierAllowance(tier) {
    if (tier === 'full') {
        const tierConfig = require('../config/tiers.json');
        return Object.keys(tierConfig.tools).length;
    }
...
// ========================================================
// Patreon token storage (for background tier revalidation)
// Stored per-user: tokens:{patreonId} → { accessToken, refreshToken, lastTierCheck }
// ========================================================

/**
 * Store Patreon OAuth tokens for a user so we can revalidate their tier later.
 */
async function setPatreonTokens(patreonId, accessToken, refreshToken) {
    await redis.set(`tokens:${patreonId}`, JSON.stringify({
        accessToken,
        refreshToken,
...
/**
 * Get stored Patreon tokens for a user. Returns null if none exist.
 */
async function getPatreonTokens(patreonId) {
    const data = await redis.get(`tokens:${patreonId}`);
    if (!data) return null;
    if (typeof data === 'string') {
...
/**
 * Update the lastTierCheck timestamp after a successful revalidation.
 */
async function updateLastTierCheck(patreonId) {
    const tokens = await getPatreonTokens(patreonId);
    if (tokens) {
        tokens.lastTierCheck = Date.now();
        await redis.set(`tokens:${patreonId}`, JSON.stringify(tokens));
    }
}

/**
 * Delete stored Patreon tokens for a user (e.g. on logout).
 */
async function clearPatreonTokens(patreonId) {
    await redis.del(`tokens:${patreonId}`);
}
